...
@property (atomic, strong, readwrite) NSURLSessionTask* httpTask;
...
- (void) cancel
{
	[UUHttpSession cancelRequest:self];
}
...
- (void) cancel:(UUHttpRequest*)request;
...
- (UUHttpRequest*) executeRequest:(UUHttpRequest*)request completionHandler:(UUHttpSessionResponseHandler)completionHandler
{
    ...
    request.httpTask = [self.urlSession dataTaskWithRequest:request.httpRequest
    ...
    [self.activeTasks addObject:request.httpTask];
    [request.httpTask resume];
    return request;
}
...
- (void) cancel:(UUHttpRequest*)request
{
	if (request.httpTask)
	{
		...
		[self.activeTasks removeObject:request.httpTask];
		request.httpTask = nil;
	}
}
...
+ (void) cancelRequest:(UUHttpRequest*)request
{
	[[UUHttpSession sharedInstance] cancel:request];
}
