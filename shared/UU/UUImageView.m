...
- (void) uuLoadImageFromURL:(NSURL*)url defaultImage:(UIImage*)defaultImage loadCompleteHandler:(void (^)(UIImageView* imageView))loadCompleteHandler
{
    ...
    else
    {
        ...
        else
        {
			...
			if (!alreadyRequested)
			{
				UUHttpRequest* httpRequest = [UUHttpRequest getRequest:url.absoluteString queryArguments:nil];
				httpRequest.processMimeTypes = NO;
				
				[UUHttpSession executeRequest:httpRequest completionHandler:^(UUHttpResponse *response)
				{
