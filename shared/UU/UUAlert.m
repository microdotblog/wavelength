...
+ (UIAlertActionStyle) actionStyleFromButtonTitle:(NSString *)button
{
	if ([button isEqualToString:@"Cancel"]) {
		return UIAlertActionStyleCancel;
	}
	else if ([button isEqualToString:@"Delete"] || [button isEqualToString:@"Destroy"] || [button isEqualToString:@"Remove"]) {
                  ...
                  buttonTitles:(NSString *)defaultButtonTitle, ...
{
	...
	UIAlertAction* alertAction = [UIAlertAction actionWithTitle:defaultButtonTitle style:[self actionStyleFromButtonTitle:defaultButtonTitle] handler:^(UIAlertAction * _Nonnull action)
	{
	...
	for (NSInteger i = 0; i < argumentArray.count; i++)
	{
		...
		UIAlertAction* alertAction = [UIAlertAction actionWithTitle:buttonTitle style:[self actionStyleFromButtonTitle:buttonTitle] handler:^(UIAlertAction * _Nonnull action)
		{
			if (completionHandler)
			{
