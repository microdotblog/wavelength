...
+ (void) authenticate:(UIViewController*)parent completionHandler:(void (^)(BOOL success, NSError* error))completionBlock
{
	...
	if (accessToken)
	{
		[UUHttpSession get:@"https://api.instagram.com/v1/users/self/" queryArguments:@{ @"access_token" : accessToken } completionHandler:^(UUHttpResponse *response)
		{
...
+ (void) logout
{
	...
	//Delete all cookies for instagram.com
...
{
	[UUHttpSession get:fullURL queryArguments:nil completionHandler:^(UUHttpResponse *response)
	{
...
+ (void) getUserMedia:(void (^)(BOOL success, NSDictionary* userMedia))completionBlock
{
	...
	[UUHttpSession get:baseAPIUrl queryArguments:@{ @"access_token" : accessToken, @"count" : @"100" } completionHandler:^(UUHttpResponse *response)
	{
		if (response.parsedResponse)
		{
			NSDictionary* sourceDictionary = response.parsedResponse;
...
- (void) showLoginController:(UIViewController*)parent completionHandler:(void (^)(BOOL success, NSError* error))completionBlock
{
	...
	[parent presentViewController:self.loginNavController animated:YES completion:nil];
}
...
- (void) setupNavigation
{
	...
	self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Cancel" style:UIBarButtonItemStylePlain target:self action:@selector(onCancelClicked:)];
}
